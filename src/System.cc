        ...
        mbDeactivateLocalizationMode(false)
{
    ...
    //----------------------------------------------------------------------------------//
    mpPointCloudMapping = make_shared<PointCloudMapping>(resolution, meank, thresh);    // point cloud 부분
    //----------------------------------------------------------------------------------//
...
cv::Mat System::TrackRGBD(const cv::Mat &im, const cv::Mat &depthmap, const double &timestamp)
{
    ...
    // Check mode change
    {
        ...
        if(mbActivateLocalizationMode)
        {
            ...
            // cout << "Is this problem? usleeeeeeeeep" << endl;
            // Wait until Local Mapping has effectively stopped
            while(!mpLocalMapper->isStopped())
            {
                usleep(1000);
            }
            // cout << "usleeeeeeeeep end" << endl;
    ...
    // Check reset
    {
    // cout << "check reset check reset check reset" << endl;
    unique_lock<mutex> lock(mMutexReset);
    if(mbReset)
    {
        mpTracker->Reset();
